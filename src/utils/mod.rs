...
use crate::{db::DB, models::user::{Claims, PersonalApplicationUser, PublicFacingUser}};
...
impl UtilFunctions {
    pub fn new(db: DB) -> UtilFunctions {
        UtilFunctions { db }
    }

    pub async fn make_public_facing_user(&self, doc: Document, token_data: TokenData<Claims>) -> Option<PublicFacingUser> {
        ...
        let mut relation = "None".to_string();
            let my_db_user = self.db.get_user_by_id(token_data.claims.userId).await?;
    ...
    pub async fn make_public_personal_facing_user(&self, doc: Document) ->  Option<PersonalApplicationUser> {
    ...
    pub fn get_profile_picture(&self, user: Document) -> String {
        if let Some(title) = user.get("profilePicture").and_then(Bson::as_str) {
            if title == "" {
                ...
                format!("https://wind-profile-pictures.s3-us-west-1.amazonaws.com/{}", title).to_string()
            }
        } else {
            "https://source.unsplash.com/random".to_string()
        }
...
pub fn verify_jwt(token: &str) -> Result<TokenData<Claims>, jsonwebtoken::errors::Error> {
    jsonwebtoken::decode::<Claims>(token, &DecodingKey::from_secret("mysecretsigningkey".as_ref()), &Validation::new(Algorithm::HS256))
}